<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">Gestion du portefeuille clients</p>
        </div>
    ...
    <div class="table-wrapper">
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td class="font-semibold text-slate-800">{{ $c->nom }}</td>
                    <td class="text-slate-600">{{ $c->pays ?? '—' }}</td>
                    <td class="text-slate-600">{{ $c->contact_nom ?? '—' }}</td>
                    <td class="text-xs text-slate-500">{{ $c->contact_email ?? '—' }}</td>
                    <td class="text-xs text-slate-500">{{ $c->contact_phone ?? '—' }}</td>
                    <td class="text-right">
                        @if($c->dossiers_count > 0)
                            <a href="{{ route('dossiers.index', ['filterClient' => $c->id]) }}"
                    ...
                    <td>
                        <div class="flex items-center gap-1">
                            <button wire:click="openEdit({{ $c->id }})" class="btn-icon" title="Modifier">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex items-center justify-between">
        <p class="text-sm text-slate-500">{{ $clients->total() }} client(s)</p>
            ...
            <div class="modal-body space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="form-group col-span-2">
                        ...
                        <input wire:model="nom" type="text" class="form-input" placeholder="PERENCO">
                        @error('nom') <p class="form-error">{{ $message }}</p> @enderror
                    </div>
                    <div class="form-group">
                        <label class="form-label">Pays</label>
                        <input wire:model="pays" type="text" class="form-input" placeholder="Cameroun">
                    </div>
                    <div class="form-group">
